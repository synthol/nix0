...
hl.bind("Print", hl.dsp.exec_cmd([[grim "$(xdg-user-dir PICTURES)/Screenshot-$(date +%Y%m%d-%H%M%S-%N).png"]]))
hl.bind("SUPER + Print", hl.dsp.exec_cmd([[geometry="$(slurp)" && grim -g "$geometry" "$(xdg-user-dir PICTURES)/Screenshot-$(date +%Y%m%d-%H%M%S-%N).png"]]))
...
hl.bind("ALT + Print", hl.dsp.exec_cmd([[geometry="$(hyprctl -j activewindow | jq -er 'select(.at and .size) | "\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"')" && grim -g "$geometry" "$(xdg-user-dir PICTURES)/Screenshot-$(date +%Y%m%d-%H%M%S-%N).png"]]))
